...
| F18 | **The first tap on the Mid and High tables is refused.** `selectedChip` defaults to the smallest denomination (`Math.min(...denoms)`) in both stores, but the engine rejects any single bet below `table_min` — so the armed $5 chip on Mid ($25 min) and the $100 chip on High ($500 min) fail every time with "That's shy of the minimum". A new player's very first action at two of three tables is a refusal; on a phone it also blocks the deal flow entirely until they find and tap a bigger chip | S | open — arm the smallest denomination ≥ `table_min` by default (`gameStore.ts`, `remoteStore.ts`), and consider hiding or greying chips below the minimum on the rail |
