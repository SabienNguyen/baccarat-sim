...
| A6 | **Derived roads marked Blue instead of Red once a run outgrew its reference column by two** — the continuation rule only asked "does the reference column reach this row"; the textbook rule compares that cell with the one above it (both present → Red, only the upper one → Blue, neither → Red). So every dragon read as a solid Blue column from its third cell, on all three roads and in the next-hand key (`roadForecast.ts` was a verbatim port, pinned to the engine by a cross-check test, so both were wrong together) | S | ✅ fixed 2026-09-13 in `scoreboard.rs` and `roadForecast.ts`; the worked-example tests never reached the case, so three regression tests now do (engine) plus two (web); the key test that had encoded the old answer is corrected |
...
| F6 | Multiplayer bust handling: a broke seat froze the whole table | M | ✅ **done 2026-07-25** — `Player::decided` now counts *can't afford the table minimum* as decided, so the deal no longer waits on a seat that has no legal move. That was a genuine deadlock: a player below `table_min` cannot place any bet, so `WaitingOnPlayers` never cleared and every other seat was stuck behind them. `SeatView` gained `broke`, which `remoteStore` maps onto its `busted` flag (previously hardcoded `false`) so the client can prompt for a rebuy or a way out. 5 new tests, including the realistic path of busting mid-session |
| F7 | Reconnect token: rejoin resumes the same seat + bankroll instead of a fresh buy-in | L | ✅ **done 2026-07-25** — a drop now *holds* the seat for 2 minutes (`HOLD`) instead of calling `table.leave`, which forfeited the bankroll and let a busted player rejoin for a free full rebuy. `Joined` carries a 32-char reconnect token; the client keeps it in sessionStorage and replays it as `rejoin` on the next connect. Fails closed: an unknown token, a live seat, or an expired hold all reclaim nothing, and a wrong token spends the same brute-force strike budget as a bad room code since it is a bearer credential for someone's money. A deliberate leave burns the token. The sweep expires holds, so an abandoned chair still frees itself. 8 new tests |
...
| F14 | **An unreachable table service read as "connection dropped"** — a close with no open before it took the same path as a mid-session drop, so a server that was never up claimed a session that never existed, on a dead-end screen. Made worse by the Fly trial ending, which took multiplayer down entirely | S | ✅ done 2026-07-25 — a distinct `offline` stage (tracked by whether the socket ever opened, not by string-matching the reason) says "Multiplayer is offline", explains single player is the same engine/shoe/roads, and leads with a gold **Play single player** CTA plus a secondary **Try again** that re-runs the connect effect rather than demanding a reload. Three tests cover never-opened vs dropped vs retry |
...
| F18 | **The first tap on the Mid and High tables is refused.** `selectedChip` defaults to the smallest denomination (`Math.min(...denoms)`) in both stores, but the engine rejects any single bet below `table_min` — so the armed $5 chip on Mid ($25 min) and the $100 chip on High ($500 min) fail every time with "That's shy of the minimum". A new player's very first action at two of three tables is a refusal; on a phone it also blocks the deal flow entirely until they find and tap a bigger chip | S | ✅ done 2026-09-13 — `defaultChip(denoms, table_min)` in `tables.ts` arms the smallest chip that clears the minimum (falls back to the smallest chip); both stores use it, so Mid opens on $25 and High on $500. The below-min top-up chip stays on the rail. 5 new/updated tests |
| F19 | **Everyone must ready up before a multiplayer coup is dealt** — a lone bettor pressing Deal could deal past seats who were still deciding their bet | S | ✅ done 2026-09-13 — `Player::ready`/`Table::ready`/`unready` (Betting only, requires a bet; a further bet or `clear_bets` un-readies); `decided` is now `sitting_out \|\| ready \|\| broke` so a betted-but-unready seat blocks the deal (solo, one-seat tables keep today's behavior: a bet there auto-readies). `Ready`/`Unready` client messages; a successful Ready announces "`name` is ready (k/n)" and, once `Table::all_ready()`, runs the same path a pressed Deal does; otherwise announces who it's waiting on. Client: `Controls` swaps the Deal button for a Ready/Unready toggle in multiplayer, `SeatsStrip` shows a ✓ on ready seats in Betting. Engine 10 / server 3 / web 8 new tests |
| F21 | **Shoe lifecycle: numbered shoes, a manual cut ceremony, cut-card-out + last hand, and a table vote for a new shoe** — see `docs/superpowers/specs/2026-09-14-shoe-lifecycle-design.md`. `PhaseTag::ShoeCut` gates every betting/deal command; the host (solo: always) drags/keys a cut card into the 416-card stack, a burn+turn reveal plays for everyone, and the roads/scoreboard reset per shoe (`Scoreboard`'s "Shoe {n}" tally). Any seat may propose a new shoe; a majority vote moves the table back to `ShoeCut` with the host cutting again. Leaving never touches the shoe (host leaving hands the cut to the next seat) | L | ✅ shipped 2026-09-14 — end-to-end probe `scripts/shoe-probe.mjs` (solo cut+burn+play, multiplayer cut/vote/re-cut). The probe also found that a non-cutter waiting on the shoe couldn't reach the HUD's Lobby button (the stage's fixed backdrop covers it) — fixed same day with a `ShoeCutStage` `onLeave` prop rendering a "Leave table" button inside the stage in multiplayer, and the probe's "B leaves" step now clicks it directly |
| F20 | **What is every other seat betting?** A multiplayer seat's stake showed only as a total ("$25 riding") — no way to tell what it was riding on without asking | S | ✅ done 2026-09-13 — `SeatView` gained `bets: Vec<PlacedBet>` (the seat's staged bets, filled for every seat in both the per-seat and public/rail views — the same money `staked` already totals, nothing new leaks). Client: a stable per-seat colour (`seatColour.ts`, 7-entry palette keyed by seat id) borders each seat's chip and tints its mini-chips; `SeatsStrip` shows compact tokens under each name ("P $25", "TIGER $5" — reusing `BetRail`'s own `betSpotLabel`), shown in Betting/Dealing and hidden once Settled (payouts take over). Every seat's bets show only in the seat strip, not on the felt. Engine 1 / web 6 new tests |
| F19 | **Multiplayer auto-advance (option B).** With the Next-hand gate gone (2026-09-15), a live table's settled felt stays until the seat acts. Consider the solo rhythm instead: linger ~3 s after the win popup, sweep, and make the sweep *stick* across other seats' re-broadcasts of the same Settled view (`remoteStore` would remember the swept `settleSeq`). Decide after feeling the current flow on a phone | S/M | open — the stickiness is client state at the same seam that produced the $0-push popup bug (S5); tread carefully |
...
| M2 | Rename at the table: a seat's name was fixed at join (lobby box only, no wire message, no engine setter) — the only way to change it was leave + rejoin, which costs the bankroll | S | ✅ done 2026-09-13 — `Table::rename`, a `rename` client message scrubbed by the same `clean_name` as a join, `me`/`rename` on the store, and your own chip in `SeatsStrip` is a click-to-edit box (Enter/blur commits, Escape cancels, blank or unchanged never sends). The new name is also saved for the next lobby visit. 9 new tests |
...
| #20 | **Large screens: `.app`'s 1560px max-width cap left the table filling only ~25% of a 2560×1440 or 3440×1440 monitor, with 8–9px labels.** | S | ✅ done 2026-09-13 (T7) — a `--ui-scale` custom property (`theme.css`) steps from 1 to 1.25 at ≥1920×1000 and to 1.5 at ≥2560×1300, driving `.app`'s max-width (1560px → 1950px → 2340px) and, via `calc(<base> * var(--ui-scale))` rounded with `round(..., 1px)`, the card size (100×143 → 125×179 → 150×215), `--road-cell` (27px → 34px → 41px), and the pixel type on the HUD, dealer line, bet spots, chips and controls. Measured (`scripts/large-probe.mjs`, `?tier=mid`, bet + deal): 1440×900 unchanged (`.app` 1440px = 100% width, card 100×143, smallest label 8px, no scroll growth); 1920×1080 `.app` fills 100% of the width (card 125×179, smallest label 10px); 2560×1440 fills 91% (card 150×215, smallest label 12px); 3440×1440 fills 68% (same card/label sizes) — both ≥50% required, no label under 12px, no viewport-vs-scroll-width mismatch (no horizontal overflow) at any of the four viewports |
| U6 | **#18: short desktop laptops (1280×720 / 1366×768) overflowed the viewport by 100-190px in the Dealt state** — the dealer bubble + roads header could scroll off the top, or the chip row + third-card table fell below the fold, and a click auto-scrolled the page to compensate. Card size and the felt's arch/gap/padding rhythm were the two biggest contributors (both height-gated only at ≥880px, so a wide-but-short laptop still rendered full 100×143 cards and 112px arches) | M | ✅ done 2026-09-13 (T6) — new `(min-width: 701px) and (max-height: 800px)` step alongside the existing 880px one: cards 100×143→76×109, bet-rail felt padding/gaps and main-bet arches (112→78px) tightened, HUD box padding/gaps tightened, road cells 27→20px, and `.board-dock` capped to the viewport (`max-height: calc(100dvh - 8px); overflow-y: auto`) with `.board` made `position: sticky; top: 0` so Big Road + tally + "Full roads" stay visible while an open Explain panel scrolls underneath. Measured (`scripts/desktop-probe.mjs`, `?tier=mid`, Bet Player → Deal, DEALT state): document height 809→720 at 1280×720 and 809→768 at 1366×768 (both now exactly viewport height, no overflow); dealer line, roads header, and chip row all fully inside the viewport at both sizes (chip row was previously off the bottom edge at y=728–778 vs a 720/768px viewport); `window.scrollY` stayed 0 after clicking "Reveal all" at every size. 1440×900 (control, >800px tall) is pixel-identical before/after — same element boxes, same 900px document height, confirming the new step doesn't touch it |
...
| P4 | **#19 — Landscape phones and tablets.** At 844×390 only one section fit at a time (the width-based 700px breakpoint gave landscape phones the desktop stack), and any tap that scrolled clipped the dealer bubble and card tops. Tablets (768×1024, 1024×768) pushed HUD + scoreboard fully below the fold with a dead void beside Big Road (fixed 27px road cells inside a much wider column) | M | ✅ done 2026-09-13 (T5, `theme.css`/`hud.css`/`betrail.css`/`scoreboard.css`) — new `@media (orientation: landscape) and (max-height: 500px)` block: `.app` regrids to 58/42 columns (`.stage` set to `display: contents` so its own children — dealer line, hands, the action bar, the bet rail — place independently: felt+hands+inline action bar left, compact HUD (bankroll/phase/limits only, goal/outcome/payouts/volume/actions collapsed) above a one-row bet rail (tabs and the hint row hidden, chips `flex-wrap: nowrap`) right; roads/board full width below); `--controls-bar-clearance` zeroed and the action bar restored to `position: static` (was fixed); `.card-stage`'s tall reserved min-height (196px, meant to absorb a dealt card without shifting the *page*) overridden to 132px since this layout has no page to shift for it. Tablets: 768–1024 wide already used the 2-column "stage / hud board" grid (≤1040px block) — added `.board { --road-cell: 34px }` in that range so Big Road/the bead plate/tallies (all derived from `--road-cell`) actually fill the column instead of floating in a dead band. Verified (`scripts/phone-probe-landscape.mjs`, before/after): at 844×390 and 780×360 all 7 core-loop elements (bankroll, dealer line, both hands, Bet Player spot, chips row, Deal) went from several clipped/off-screen to **all `fullyVisible: true`** with `scrollWidth === innerWidth` (no horizontal overflow) at every one of the 4 viewports. No regression: Mid Betting doc height at 390×664 unchanged at **1276px** (`phone-probe-p12.mjs`, before and after identical) and 1440×900 unchanged (`scrollWidth`/`scrollHeight` both exactly 1440×900, no overflow) |
...
| P12 | **Phone type and tap targets measured too small (2026-09-13, headless Chromium, 390×844 and 360×780, DPR 2).** Display-font text at **8px**: `.hud-box-label`, `.hud-action` (Reset bank / Lobby), `.spot-payout` ("PAYS 1 TO 1"), `.chip-face` ("$5"); at **9px**: `.dealer-tag`, `.controls .btn` (Deal / Watch hand / Reveal all), `.clear-bets`, `.road-info-btn`, `.full-roads-btn`, `.hud-goal-pct`; at **11px**: `.spot-name`, `.seg-half` (MAIN BETS / BONUS), tallies, road headings. Tap targets under 44px: Reset bank 89×30, Lobby 59×30, Mute 33×26, Music 23×26, volume slider 95×16, road "?" 18×18, bonus "i" 24×24, MAIN/BONUS tabs 127×36, Full roads 102×35, and the home-screen learn links (13px tall). Page is exactly viewport-wide in every state (no overflow regression) | M | ✅ done 2026-09-13 (T2, scoped to `theme.css`/`hud.css`/`betrail.css`/`controls.css`/`dealer.css`/`scoreboard.css`/`home.css`/`Controls.tsx` only) — phone type floor applied (8→10, 9→11, 11→12) across every named element plus a few more found by the same rule (`.spot--side .spot-name`/`.spot-payout`, `.seg-badge`); 44px hit areas via `min-height`/padding on Reset bank, Lobby, Clear bets, MAIN/BONUS tabs, Full roads, home-back and the home-learn links; the pinned bar is now genuinely one row in every phase, including Betting's 5-button case (Deal stays primary — gold, extra flex share — while Watch hand/Reveal all/New Shoe/Next hand get CSS-only short labels on phones via a visually-hidden-full-label / aria-hidden-short-label pair, so `getByRole` and screen readers still see the full name). `--controls-bar-clearance` now reflects the real measured height (62px + safe-area) instead of the old 114px assumption, which is most of why the page got *shorter* despite every target growing. Measured (`scripts/phone-probe-p12.mjs`, iPhone 14 390×844 + 360×780 Android-like, states: Betting-no-bets/5-buttons, Betting-with-bet, Dealing, Settled): bar height 114→62 in Betting-no-bets (unchanged 62 elsewhere); in-fence under-44 targets 5→0 and in-fence under-10px labels 24→0 in every state; Mid Betting document height at 390×664 1200→**1190** (down, per the task's own bar). **Update 2026-09-13 (T2b)** — ✅ fully done, the five-file residual closed: Mute/Music (`volume.css`) and road "?"/bonus "i" (`glossary.css`/`bonusinfo.css`) keep their small visible chips (unchanged real size — growing them in-flow either widened the shared HUD row or, for road "?", added real height to every road heading and blew the Mid Betting budget) but each gets an invisible out-of-flow `::before` hit-expander (`position: relative` + negative `inset`) that pads the real tappable area to 44×44 without moving anything around it; the volume slider's own box grows to a real 44px tall (the shared HUD row was already that tall for other reasons) with a `margin-block: 0` fix for the ~4px UA default margin that was otherwise sneaking into the row's height, plus a ≥28px thumb. "Ask the dealer" flip buttons/tag (`dealerflip.css`) and the hand `Total` label (`cards.css`) got real in-flow floor/size fixes (9→11px, 8→10px, min-height 44) since they only render during Dealing/Settled, which has no page-height budget to protect. Also fixed: `controls.css`'s Deal button rendered the same red as every other `.controls .btn` in every Betting/Dealing/Settled screenshot — `.controls .btn` (0,2,0 specificity) beat the single-class `.btn--primary` (0,1,0) regardless of source order, so Deal's gold face never actually applied; changed the primary-face rule to `.controls .btn--primary` (matching specificity, later in source) to fix the cascade. `scripts/phone-probe-p12.mjs` was extended (not in the original file fence, but required to verify any of this): it now credits a `::before` hit-expander's real effective hit area instead of only the host element's own box, exempts genuinely-inline prose links (dealer-dialogue glossary terms, per the WCAG 2.5.5/2.5.8 "target in a sentence" exception) from the 44px check, and asserts Deal's computed `background-color` equals the resolved `--gold` token in Betting-with-a-bet. Measured (iPhone 14 390×844 + 360×780 Android-like, same four states): in-fence AND previously-out-of-fence under-44/under-10px lists are both **empty in every state at both widths** (zero exceptions remain); Mid Betting document height at 390×664 holds at **1190** (unchanged — an interim slider-height fix regressed it to 1198, root-caused to the UA margin above, then a road-"?" real-size regression brought it to 1194, both fixed before landing); `npx vitest run` 516/516 passing, `npx tsc --noEmit` clean |
| P13 | **Pinned action bar covers content at the bottom edge.** On phones `.controls` is `position: fixed` (P2) and `.app` reserves 64px of bottom padding, but anything scrolled to the bottom edge of the viewport — a bet spot, the Banker hand — sits *under* the bar: a headless tap on "Bet Player" was intercepted by "Reveal all"/"Explain". A thumb lands the same way | S | ✅ done 2026-09-13 — `--controls-bar-clearance` (114px, the bar's real 2-row height, not the assumed 64px) now drives both `html { scroll-padding-bottom }` and `.app`'s own bottom padding; `.controls` is `pointer-events: none` with `pointer-events: auto` restored on its `.btn`s, so dead bar-box space falls through to the felt. Probe: tapping "Full roads" at max scroll went from landing on "Reveal all" (before) to opening the roads modal (after); known residual gap — a shortened viewport (390×600) still overlaps a real Bet Banker spot with a real Betting-phase bar button, tracked separately (button-on-button, not fixable by pointer-events). **Update 2026-09-13 (T2/P12):** the bar is now one row in every phase (short labels on phones), so `--controls-bar-clearance` was re-measured down from 114px to the real 62px (+ safe-area) |
| P14 | **Deal-flow on phones (reported 2026-09-13, not yet reproduced in a screenshot).** Owner reports "fundamental issues with UI flow" once cards are dealt. Blocked from measuring by F18 (the first tap is refused on Mid/High). Suspects, unconfirmed: cards landing above the fold when the user is down at the chip rail; squeeze drag vs. page scroll; the pinned bar over the Banker hand (P13); auto-settle/auto-advance timing on a small screen | M | ✅ done 2026-09-13 (T8a, `scripts/phone-probe-p14.mjs`, iPhone 14 390×664, Mid table, solo). Re-measured on ed9c7a2 (T2's parent) as the "before" baseline instead of trusting the stale numbers: Player-hand top 287→405 across the coup (118px, matching the reported jump), doc height 1171 (Betting) → 1322 (Settled, +151px), and the win popup did land on the Player hand (`popupIntersectsHand: true`). **1 — no HUD growth at settle:** chose *reserve a fixed-height slot up front* over folding into the Phase box, because the payouts row's line count varies with how many bets were placed and a folded single line can't represent that; `Hud.tsx`'s Outcome box and payouts list are now always mounted (blank/empty during Betting/Dealing) and `hud.css` gives both a **fixed height** in the phone media query (34px / 40px with `overflow-y: auto` for the rare hand with more bets than the slot shows at once) instead of `display: none` — off-phone they still collapse via `data-has-outcome`/`data-has-payouts` attributes, unchanged from before. **2 — popup off the cards:** `App.tsx` wraps `DealerLine` and `WinPopup` in a new `.dealer-slot` (App.tsx, dealer.css) so the popup can anchor to the dealer line's own box on phones (`winpopup.css`, ≤700px: `position: absolute; bottom: 2px` inside it) instead of a viewport-`top: 58%` that landed on the cards; desktop is unaffected (`position: fixed` there ignores the DOM move). **3 — card size step — dropped from this task** by owner direction: a follow-up will replace the Dealing-phase phone layout with a dedicated full-viewport peel stage, making a resize here wasted work. **4 — auto-advance:** new `web/src/autoAdvance.ts` (unit-tested, `autoAdvance.test.ts`) — `autoAdvanceMs()` reads `matchMedia("(pointer: coarse)")` once per settle and adds 2000ms (3000→5000) on a coarse pointer; `isRecentlyTouched()` gates two rescheduling timers in `App.tsx` (a `pointerdown`-tracking ref, `pointerType === "touch"`) so the sweep/clear defer in 250ms polls while a touch landed within the last second, instead of sweeping the felt out from under a thumb. **5 — bet rail reachable in Settled:** verified, not changed — no auto-scroll exists and the pinned bar's Deal/Next-hand button was already the one-tap way forward; probe confirms it 44×44 and tappable in Settled. **Also fixed, not one of the 5 items but required to hit the felt-stability bar:** `dealer.css`'s phone bubble reserved only 52px (1 line), but common lines — "Bets down — $X riding...", a natural call, the settle line — measure 70.5px (2 lines), so the felt still jumped ~19px at bet-placement and again at settle; bumped the reserved `min-height` to 71px. After: Player-hand top spread **0px** across Betting→Bet placed→Dealing→Settled→auto-advance; popup never intersects either hand; doc height 1276 (Betting) → 1311 (Dealing, +35px, the pre-existing card/total-badge growth, out of scope) → typically 1309–1311 at Settled; `scrollWidth` stays 390 throughout; Deal is 79×44 and hit-tests to itself in Settled. **Residual, not fixed here:** on hands where the pre-existing "you would've won" BonusNudge appears (a different, deliberate feature, not named in this task), doc height reaches 1382 (+106px over Betting) because the nudge banner isn't reserved space — tracked as a new gap, not part of P14's scope. |

| T8b | **Peel stage on phones + reachable peel + peek lens (owner direction 2026-09-13).** "Once the bets are in only the view of the cards matters ... bring the cards up to the front of the player's view" and "since we are peeling with the finger the card is under the finger, so make the radius in which the card is peelable larger than the face" | M | ✅ done 2026-09-13 — new `PeelStage.tsx`/`peelstage.css`: on a coarse pointer or ≤700px viewport (`phoneLike.ts`), once the phase flips to Dealing and the deal fly-in has had 700ms to land (`DEAL_SETTLE_MS`, `App.tsx`), a fixed full-viewport stage takes over — compact `DealerLine` (new `compact` prop), Player above Banker (reusing `Hand`/`SqueezeCard` unchanged, so squeeze/peek/flip logic is identical, not a parallel implementation), the dealer-flip ask, all under the pinned action bar (`z-index: 40` vs. the bar's 45). Cards render at 112px (steps to 104px under 376px wide only once a hand shows 3 cards — 2-card hands never need it). Body **and html** scroll-locked while mounted, restored on unmount; unmounts the instant the phase leaves Dealing. **Peel reach:** `squeeze.ts`'s new `clampToRect` maps a grab starting outside the card's face to the nearest point on it before any fold math runs, and `SqueezeCard`'s wrapper grows its hit box 28px past the card on every side via padding + equal negative margin (`.squeeze-wrapper` in `cards.css`) — invisible, so neighbouring layout is untouched. **Peek lens:** on a coarse pointer, while a peek is held, a 120×120px `.peek-lens` floats 72px above the finger (clamped to the viewport) showing a true 2x magnifier — a second `<Card>` render offset so whatever sits under the actual finger lands at the lens's centre, not a fixed corner crop — removed on release; never rendered on a fine pointer. Chosen over drawing from the WebGL overlay's canvas: simpler, and works identically whether the GL squeeze is active or not (in GL mode `fold` state, otherwise write-only for the overlay's port, is also kept live once a peek is held, so the lens's ordinary Card/Peel path has something to render — a bounded cost, only for the lens's own window). Probe (`scripts/phone-probe-p14b.mjs`, 390×844 and 360×780): stage covers the viewport; card widths 112/112/112/112; `scrollWidth` matches the viewport; `html`/`body` both computed `overflow: hidden`; a touch drag starting 20px above the first Player card changed the dealer line from "No more bets. Cards are out — squeeze 'em." to "Take your time — bend that corner." (a peek); `.peek-lens` appeared above the pointer and disappeared on release; Reveal all → Settled unmounted the stage. Tests: `phoneLike.test.ts`, `PeelStage.test.tsx` (mount/unmount, body+html overflow, card-count sizing step), `SqueezeCard.test.tsx` additions (reach maps to the nearest point on the card; lens shows only on a coarse pointer during a held peek, never on a fine one). **Update 2026-09-13:** peel reach removed for now (owner: it was messing up the cards); revisit. Peek lens removed entirely (owner: "why is that magnifying glass coming up — get rid of that"). The stage itself is now an overlay (backdrop + scaled-in hands) over the still-mounted felt rather than a separate view that swapped it out — see `App.tsx`/`PeelStage.tsx`/`peelstage.css`. **Update 2026-09-13 (P14c):** owner reported the deal was still visible on the felt before the overlay took over — the 700ms `DEAL_SETTLE_MS` wait (above) let the inline fly-in play in full view first. Removed: the overlay now mounts (and the inline hands are hidden via `.card-stage--peeling`) the instant `snapshot.phase` becomes `"Dealing"`, so the deal-in fly-in plays inside the overlay's own `Hand`s from the start; `PEEL_EXIT_MS`'s fade-out is unchanged. No `PeelStage.tsx` change was needed — its `Hand`s already replay `cards.css`'s `deal-in` keyframe on their own mount, independent of the inline felt. Probe (`scripts/phone-probe-p14c.mjs`, 390×844 and 390×664, `PORT=5173`): `.peel-backdrop` appeared 69–71ms after the Deal tap (budget 100ms), and no inline `.card-stage .card` was ever visible while the phase read Dealing, at either viewport. |

## Battery / power (phones)

Read-only audit 2026-09-13 after the owner reported the app "destroying
battery life" on a phone. Nothing here is gameplay; all of it runs whether or
not a hand is in progress.

| # | Item | Effort | Notes |
|---|------|--------|-------|
| B1 | **The felt repaints the whole screen every frame, forever.** `theme.css:74` — `animation: felt-swirl 40s ease-in-out infinite alternate` on `<body>` animates `background-position` on three full-viewport radial gradients with `background-attachment: fixed`. That is not compositor-driven: the entire background re-rasterises ~60×/s on the home screen, in the lobby, at the table and in the roads dialog. Already off under `prefers-reduced-motion`. The single largest power cost in the app and it carries no information | S | ✅ done 2026-09-13 — gated to `@media (pointer: fine) and (min-width: 701px)`; static felt on phones/tablets and any coarse-pointer device |
| B2 | **Audio never sleeps.** The ambience (`sfx.ts:270+`) is a looped noise buffer + LFO + a `setTimeout` "sparkle" chain, plus the optional lounge loop, on a Web Audio graph started when a table mounts. There is no `visibilitychange` handler anywhere in `web/src`, so locking the phone or switching apps leaves the `AudioContext` rendering (browsers throttle timers and rAF in the background — not audio). Confirm whether mute only zeroes the master gain (graph keeps running) or suspends the context | S | ✅ done 2026-09-13 — mute confirmed gain-only; `sfx.ts` gained `suspendAudio()`/`resumeAudio()`, wired into `setMuted`; `sleep.ts` suspends/resumes on hide/show |
| B3 | Infinite CSS pulses: `total-glow` (900 ms, cards.css:322), `victory-glow` (victory.css:29), `seat-wait` (multiplayer.css:363). Small individually; check which properties they animate — a `box-shadow`/`filter` pulse is paint, not compositor. `seat-wait` already honours reduced motion | S | ✅ done 2026-09-13 — audited: all three already animate only `transform`/`opacity` (no box-shadow/filter in any keyframe); no CSS change needed. `.hidden * { animation-play-state: paused }` (theme.css, from B5) now also freezes them when the tab is hidden |
| B4 | The WebGL squeeze overlay (`CardGLOverlay.tsx:235`) runs a `requestAnimationFrame` loop for as long as it is mounted; it only calls `engine.render` during drag/settle/flip, so idle frames are cheap, but the tick keeps the main thread awake. Not yet confirmed whether the overlay is mounted only for the duration of a squeeze or for the life of a face-down card | S | ✅ done 2026-09-13 — confirmed: mounted only for one squeeze gesture (`SqueezeCard`'s `glActive`, grab → settle/flip `onDone`), never for a face-down card's whole life; loop logic extracted to a tested `createFrameLoop` helper, still starts on mount/grab and stops on unmount or a completed settle/flip |
| B5 | No `visibilitychange` handling at all (see B2): CSS animations, audio and rAF all continue when the app is backgrounded or the screen is locked; standalone PWA mode makes this worse because there is no browser tab to throttle | S | ✅ done 2026-09-13 — `web/src/audio/sleep.ts` (`installSleepOnHide`), installed once in `App.tsx`; toggles `<html class="hidden">`, paired with theme.css's `.hidden * { animation-play-state: paused !important; }` |
...
| G4 | Privacy-friendly analytics (GoatCounter) + events: first-hand, returning-visitor, victory/bust, room-link vs lobby join | S | ✅ done 2026-09-10 — GoatCounter live (`baccarat-sim.goatcounter.com`: script tag in `index.html`, CSP allow in `server/src/main.rs`); all events instrumented: visit/victory/bust/share (2026-07-24) + `first-hand` (once per load, solo and live) + `join-room-link` vs `join-lobby` |
...
- **2026-09-13 (spectator mode, F17):** Mapped the multiplayer path end to
  ...
  web 499 green.
- **2026-09-13 (phone audit, read-only):** Measured the phone build in
  headless Chromium at 390×844 / 360×780 (type sizes and tap-target boxes,
  logged under P12) and found two flow blockers on the way: the fixed action
  bar intercepts taps at the bottom edge (P13) and the default chip is below
  the Mid/High minimum (F18). Read the code for what burns power with no
  hand in progress — a full-screen `background-position` animation on
  `<body>`, an audio graph with no `visibilitychange` handling, three
  infinite pulses and an always-ticking rAF loop (B1–B5). The PWA question
  (install without a store) is answerable with what's already there — the
...
- **2026-09-15 (no more Next hand at live tables):** "Settled" at a shared
  table is the seat's own result still on the felt — the server's table is
  already open — but the client gated Sit out, Ready and New shoe on
  `phase === "Betting"`, so a purely cosmetic Next press stood between a
  finished hand and every action (and any other seat's push undid it).
  Controls now treat Settled as open at a live table; `sitOut` and
  `requestNewShoe` sweep the felt locally before sending, as `stake` already
  did; the Next-hand button is gone. Solo tables unchanged (they auto-advance).
  Auto-advance for live tables logged as F19.
