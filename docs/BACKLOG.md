...
| E2 | **Scoreboard recomputed from full history on every wasm call** — `derive_scoreboard` (O(n) over all rounds, all 5 roads) runs on every `place_bet`/`peek`/`reveal`/`settle`, ~8-10× per coup, i.e. O(n²) aggregate over a session, with the whole snapshot copied across the FFI each time. **Server angle (audit 2026-07-24):** on the multiplayer server this also runs per seated player per `broadcast()` — i.e. on every accepted command from any seat — and `Table.history` is never capped and has no room TTL beyond "empty room reaped," so a long-lived/heavily-played room is an unbounded CPU/growth vector a client can drive with cheap spam (`ClearBets`/`SitOut`/`Peek`) | M | `session.rs:253,378`, `table.rs:584,607`, `rooms.rs broadcast` — cache per settled round + invalidate on history change, and/or cap `history` to what the windowed roads actually need |
