...
| Y6 | **The only "leave table" control is unreachable while the shoe-cut ceremony is up.** `ShoeCutStage`'s full-viewport backdrop (`position: fixed; inset: 0; z-index: 40`) sits over the HUD, which never raises its own stacking outside the phone breakpoint's pinned action bar — so a non-cutter waiting on "Waiting for &lt;name&gt; to cut the shoe" (or anyone during the New-Shoe vote/re-cut) cannot click **Lobby** to leave; the click is intercepted by `.shoe-stage-content`. Found by `scripts/shoe-probe.mjs` (`mp: B can click Lobby to leave while waiting on the shoe cut`), which times out on the click | S | open — give the Lobby/leave control a z-index above `.shoe-stage` (or render it inside the stage), and confirm keyboard focus can already reach it (Tab order may already escape the modal, which is its own a11y gap — see Y4) |
