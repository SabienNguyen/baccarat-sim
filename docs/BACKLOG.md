...
| # | Item | Effort | Notes |
|---|------|--------|-------|
| B1 | **The felt repaints the whole screen every frame, forever.** `theme.css:74` — `animation: felt-swirl 40s ease-in-out infinite alternate` on `<body>` animates `background-position` on three full-viewport radial gradients with `background-attachment: fixed`. That is not compositor-driven: the entire background re-rasterises ~60×/s on the home screen, in the lobby, at the table and in the roads dialog. Already off under `prefers-reduced-motion`. The single largest power cost in the app and it carries no information | S | open — gate it to `(pointer: fine)` (or drop it on `max-width: 700px`), or move the drift to a `transform` on a fixed pseudo-element so it composites. Measure with Chrome's rendering stats / `about:tracing` before and after rather than assert |
| B2 | **Audio never sleeps.** The ambience (`sfx.ts:270+`) is a looped noise buffer + LFO + a `setTimeout` "sparkle" chain, plus the optional lounge loop, on a Web Audio graph started when a table mounts. There is no `visibilitychange` handler anywhere in `web/src`, so locking the phone or switching apps leaves the `AudioContext` rendering (browsers throttle timers and rAF in the background — not audio). Confirm whether mute only zeroes the master gain (graph keeps running) or suspends the context | S | open — `document.addEventListener("visibilitychange")` → `ctx.suspend()` + `stopAmbience()` when hidden, resume on return; suspend on mute too |
| B3 | Infinite CSS pulses: `total-glow` (900 ms, cards.css:322), `victory-glow` (victory.css:29), `seat-wait` (multiplayer.css:363). Small individually; check which properties they animate — a `box-shadow`/`filter` pulse is paint, not compositor. `seat-wait` already honours reduced motion | S | open — prefer `opacity`/`transform`, and stop them when the document is hidden |
| B4 | The WebGL squeeze overlay (`CardGLOverlay.tsx:235`) runs a `requestAnimationFrame` loop for as long as it is mounted; it only calls `engine.render` during drag/settle/flip, so idle frames are cheap, but the tick keeps the main thread awake. Not yet confirmed whether the overlay is mounted only for the duration of a squeeze or for the life of a face-down card | S | open — confirm mount lifetime; start the loop on grab, stop on release/settle |
| B5 | No `visibilitychange` handling at all (see B2): CSS animations, audio and rAF all continue when the app is backgrounded or the screen is locked; standalone PWA mode makes this worse because there is no browser tab to throttle | S | open — one document-level listener that pauses audio, stops ambience, and adds a `.hidden` class that switches animations off |
